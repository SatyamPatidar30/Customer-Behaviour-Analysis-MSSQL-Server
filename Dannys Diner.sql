...
WITH customer_first_purchase AS(
       SELECT s.customer_id, MIN(s.order_date) AS customer_first_purchase
	   FROM dbo.sales s 
	   GROUP BY s.customer_id
)
SELECT cfp.customer_id, cfp.customer_first_purchase, m.product_name
FROM customer_first_purchase cfp
INNER JOIN dbo.sales s ON s.customer_id = cfp.customer_id 
AND s.order_date = cfp.customer_first_purchase
...
JOIN dbo.sales s ON s.customer_id = fpam.customer_id 
...
JOIN dbo.sales s ON s.customer_id = lpbm.customer_id 
AND s.order_date = lpbm.last_purchase_date
JOIN dbo.menu m ON m.product_id = s.product_id

-- 8. What is the total items and amount spent for each member before they became a member?

SELECT s.customer_id, COUNT(*) AS total_item
FROM dbo.sales s
JOIN dbo.members mb ON mb.customer_id = s.customer_id
WHERE s.order_date < mb.join_date
GROUP BY s.customer_id
